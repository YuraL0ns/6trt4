...
class ImageProcessor:
    ...
    def remove_to_exif_and_rotate(self, image_path: str, output_path: Optional[str] = None) -> str:
        ...
        # ВАЖНО: Если мы нашли EXIF ориентацию, применяем её вручную для гарантии
        # ImageOps.exif_transpose() может не работать в некоторых случаях
        if exif_orientation and exif_orientation != 1:
            logger.info(f"Applying manual EXIF orientation: {exif_orientation}")
            try:
                ...
                logger.warning(f"Manual orientation failed: {str(manual_error)}, trying ImageOps.exif_transpose()")
                # Fallback на ImageOps.exif_transpose()
                try:
                    img_before = img.copy()
                    img = ImageOps.exif_transpose(img)
                    new_size = img.size
                    if original_size != new_size or img_before.size != img.size:
                        logger.info(f"EXIF orientation applied via exif_transpose fallback: {original_size} -> {new_size}")
                        orientation_applied = True
                except Exception as transpose_error:
                    logger.error(f"Both manual and exif_transpose failed: {str(transpose_error)}")
        else:
            # Если ориентации нет или она нормальная, пробуем ImageOps.exif_transpose() на всякий случай
            try:
                img_before = img.copy()
                img = ImageOps.exif_transpose(img)
                new_size = img.size
