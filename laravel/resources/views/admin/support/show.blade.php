        ...
        <div class="lg:col-span-2">
            <x-card>
                ...
                @if($ticket->status === 'open')
                    <form action="{{ route('admin.support.add-message', $ticket->id) }}" method="POST">
                        @csrf
                        <x-textarea name="message" rows="4" placeholder="Ваш ответ..." required />
                        <div class="flex space-x-3 mt-3">
                            <x-button type="submit">Отправить</x-button>
                            <x-button href="{{ route('admin.support.close', $ticket->id) }}" variant="outline" type="button">
                                Закрыть тикет
                            </x-button>
                        </div>
                    </form>
            ...
            <x-card title="Информация">
                <div class="space-y-3">
                    <div>
                        ...
                        <p class="text-white font-semibold">{{ $ticket->user->full_name }}</p>
